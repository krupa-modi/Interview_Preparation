...
Used to identify unnecessary component re-renders and performance issues.

### Network Monitoring

Check API requests in the browser's Network tab.

## Interview Answer

"I debug applications using browser DevTools, breakpoints, console methods, React Developer Tools, React Profiler, and the Network tab. These tools help identify rendering issues, API failures, and performance bottlenecks efficiently."
...
A Progressive Web App (PWA) is a web application that behaves like a native mobile application.

It can be installed on a device, work offline, and provide a fast user experience.
...
## 1. What exactly causes a hydration error in Next.js?

A hydration error occurs when the HTML generated on the server does not match the HTML generated by React on the client during hydration.

---

## 2. What differs between server HTML and client render?

Common causes:

* Using `Math.random()`
* Using `Date.now()`
* Accessing `window` or `localStorage`
* API data changing between server and client
* Conditional rendering based on browser-only values

React expects identical markup on both sides.

---

## 3. Why does a component re-render even when props don't change?
...
* State changed
* Context changed
* New object/function reference was passed

React re-runs the component function unless prevented by `React.memo`.

---

## 4. What is reference equality?

Objects, arrays, and functions are compared by memory reference, not value.
...
New references trigger re-renders even if data looks identical.

---

## 5. What happens internally when you call setState?

React:

1. Creates an update object
2. Places it in an update queue
3. Schedules a re-render
4. Reconciles changes
5. Updates the DOM if required

State updates are asynchronous.

---

## 6. What is batching and scheduling?

React groups multiple state updates into a single render for better performance.
...
React batches these updates and renders once.

---

## 7. How does React decide what to re-render?

React:

1. Creates a new Virtual DOM tree
2. Compares it with the previous tree
3. Finds differences (diffing)
4. Updates only changed parts

---

## 8. Is it Virtual DOM magic?
...
A stale closure occurs when a function captures an old state value and continues using it later.
...
The callback may use an outdated `count`.

---

## 10. Why does stale closure break logic?
...
The function uses old values instead of the latest state.

---

## 11. How would you debug performance issues in a large React app?

Process:

1. Identify slow screens
2. Use React Profiler
3. Check unnecessary renders
...
These hooks themselves have overhead.

---

## 14. Why is over-optimization bad?
...
## 15. How do Server Components change data fetching?
...
No need for useEffect fetching.

---

## 16. Why are Server Components important?
...
Important concept in Next.js 14+.

---

## 17. Why do bad keys break list rendering?
...
## 18. What real bugs happen with bad keys?
...
* State jumping between items

---

## 19. How would you scale this component to handle 1M users?
